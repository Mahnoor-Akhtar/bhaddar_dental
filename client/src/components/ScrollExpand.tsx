// Design philosophy: Warm Editorial Smile — use calm editorial composition, sea-glass overlays, quiet motion, and readable care-focused messaging.
...
type ScrollExpandConfig = Required<Pick<ScrollExpandProps, "startWidth" | "startHeight" | "startRadius" | "endRadius" | "mediaZoom" | "scrollDistance" | "holdDistance" | "smoothing" | "overlayScrim" | "useWindowScroll" | "enabled">>;
...
export default function ScrollExpand({
  ...
  const propsRef = useRef<ScrollExpandConfig>({ startWidth, startHeight, startRadius, endRadius, mediaZoom, scrollDistance, holdDistance, smoothing, overlayScrim, useWindowScroll, enabled });
  ...
  useEffect(() => {
    ...
    const kick = () => { if (!running) { running = true; if (!frame) frame = requestAnimationFrame(tick); } };
    const onScroll = () => {
      target = readProgress();
